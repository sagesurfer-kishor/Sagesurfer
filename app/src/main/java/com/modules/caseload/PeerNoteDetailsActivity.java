...
public class PeerNoteDetailsActivity extends AppCompatActivity implements View.OnClickListener, CaseloadPeerNoteRejectReasonsAdapter.CaseloadListAdapterListener {
    private static final String TAG = PeerNoteDetailsActivity.class.getSimpleName();
    ...
    /*@BindView(R.id.textview_toolbar_approve)
    TextView textViewApprove;
    @BindView(R.id.textview_toolbar_reject)
    ...
    public String subject = "", date = "", time = "", note = "", type_of_contact = "", duration = "", next_steps = "", resources_needed = "";
    Toolbar toolbar;
    public ArrayList<CaseloadPeerNote_> caseloadPeerNoteArrayList = new ArrayList<>();
    private String action = "";
    //private  int note_status = -1;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (CheckRole.isCoordinator(Integer.parseInt(Preferences.get(General.ROLE_ID)))) {
            ...
            //textViewApprove.setVisibility(View.GONE);
            //textViewReject.setVisibility(View.GONE);
            setViewLogButtonBackground();
            if (action.equalsIgnoreCase(Actions_.GET_PENDING)) {
                buttonViewLog.setVisibility(View.VISIBLE);
            } else if (action.equalsIgnoreCase(Actions_.GET_APPROVED)) {
                buttonViewLog.setVisibility(View.VISIBLE);
                imageViewAddAmendment.setVisibility(View.VISIBLE);
            } else if (action.equalsIgnoreCase(Actions_.GET_REJECTED)) {
                buttonViewLog.setVisibility(View.VISIBLE);
            ...
            setViewLogButtonBackground();
            if (action.equalsIgnoreCase(Actions_.GET_APPROVED)) {
                buttonViewLog.setVisibility(View.VISIBLE);
            }
        }
    }
    ...
    protected void onResume() {
        ...
        if (!isFromNotification) {
            ...
            if (CheckRole.isCoordinator(Integer.parseInt(Preferences.get(General.ROLE_ID)))) {
                if ((action.equalsIgnoreCase(Actions_.GET_PENDING) || action.equalsIgnoreCase(Actions_.GET_APPROVED)) && caseloadPeerNoteArrayList.get(0).getReject_cnt() > 0) {
                    imageViewComment.setVisibility(View.VISIBLE);
                    textViewCommentCount.setVisibility(View.VISIBLE);
                }
            }
            ...
            if (CheckRole.isSupervisor(Integer.parseInt(Preferences.get(General.ROLE_ID)))) {
                if (action.equalsIgnoreCase(Actions_.GET_PENDING) && caseloadPeerNoteArrayList.get(0).getIs_on_leave() == 0) { //isOnLeave Condition
                ...
                } else if (action.equalsIgnoreCase(Actions_.GET_PENDING) && caseloadPeerNoteArrayList.get(0).getIs_on_leave() == 1) {
                ...
                if (action.equalsIgnoreCase(Actions_.GET_PENDING) && caseloadPeerNoteArrayList.get(0).getIs_on_leave() == 1) { //isOnLeave instanceAdmin
                    linearLayoutLeaveHeader.setVisibility(View.VISIBLE);
    ...
    private void getPeerCaseloadNoteDetails() {
        ...
        if (CheckRole.isSupervisor(Integer.parseInt(Preferences.get(General.ROLE_ID))) || CheckRole.isInstanceAdmin(Integer.parseInt(Preferences.get(General.ROLE_ID)))) {
