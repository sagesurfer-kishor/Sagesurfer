...
import com.modules.selfgoal.werhope_self_goal.adapter.CustomGoalsAdapter;
...
public class CustomGoalsFragment extends Fragment {
    private static final String TAG = CustomGoalsFragment.class.getSimpleName();
    ...
    private void getCustomGoalList() {
        HashMap<String, String> requestMap = new HashMap<>();
        requestMap.put(General.ACTION, Actions_.CUSTOM_GOAL);
        ...
        if (requestBody != null) {
            try {
                ...
                if (response != null) {
                    goalArrayList = SelfGoal_.parseDefaultsSelfGoal(response, Actions_.CUSTOM_GOAL, activity.getApplicationContext(), TAG);
                    if (goalArrayList.size() > 0) {
                        if (goalArrayList.get(0).getStatus() == 1) {
                            showError(false, 1);
                            CustomGoalsAdapter customGoalsAdapter = new CustomGoalsAdapter(activity, false, goalArrayList);
                            listView.setAdapter(customGoalsAdapter);
                        } else {
                            JSONObject jsonObject = new JSONObject(response);
                            if (jsonObject.has(Actions_.CUSTOM_GOAL)) {
                                JSONArray jsonArray = jsonObject.getJSONArray(Actions_.CUSTOM_GOAL);
